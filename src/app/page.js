...
const applyNMS = (boxes, iouThreshold = 0.4) => {
  ...
  for (const box of boxes) {
    ...
    for (const s of selected) {
      if (calculateIoU(box, s) > iouThreshold) {
        keep = false;
        break;
      }
    }
...
export default function CarbonDashboard() {
  ...
  // ยอด Total สะสม (นับเฉพาะคน/ชุดที่ไม่เคยนับ)
  ...
  const renderLoop = () => {
    ...
    persons.forEach((person) => {
      ...
      const itemsList =
        person.items.length > 0 ? ` [${person.items.join(", ")}]` : "";
      const text = `👤 Person #${person.id} (${person.carbon.toFixed(1)} kg)${itemsList}`;

      ctx.fillStyle = "#eab308";
      ctx.fillRect(x, y - 35, ctx.measureText(text).width + 30, 35);
      ctx.fillStyle = "#000000";
  ...
  // --------------------------------------------------------
  // ลูปที่ 2: AI + Tracking + บันทึกยอดสะสม (แก้บั๊กตรงนี้!)
  // --------------------------------------------------------
  const runAILoop = async () => {
    ...
    try {
      ...
      // 🌟 แก้ไขสำคัญ: สั่งรัน AI "ทีละตัวตามลำดับ (Sequential)" เพื่อไม่ให้เบราว์เซอร์แย่ง Buffer กัน 🌟
      ...
      for (let index = 0; index < 8400; index++) {
        ...
        if (maxConf > 0.6) {
          const xc = outClothes[0 * 8400 + index],
            yc = outClothes[1 * 8400 + index],
            w = outClothes[2 * 8400 + index],
            h = outClothes[3 * 8400 + index];
          currentClothes.push({
            item: CLOTHES_CLASSES[classId],
            conf: maxConf,
            xc,
            yc,
            w,
      ...
      latestUIData.current = {
        persons: updatedTracks,
        ...
        <div className="w-full lg:w-[380px] flex flex-col gap-4">
          <div className="flex justify-between items-start px-2">
            <div>
              ...
              <p className="text-neutral-400 text-sm mt-1">
                Smart Tracking & Accumulation
              </p>
            </div>
            <button
              onClick={resetCounter}
              className="bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/30 text-xs font-bold px-3 py-2 rounded-xl transition-all shadow-md cursor-pointer"
            >
          ...
          <div className="bg-gradient-to-br from-emerald-900/40 to-teal-900/20 p-6 rounded-2xl border border-emerald-500/20 shadow-lg relative overflow-hidden">
            ...
            <p className="text-xs text-neutral-400 mt-2 italic">
              *ล็อกยอดอัตโนมัติ ไม่บวกซ้ำคนเดิม
            </p>
          </div>
            ...
            {currentTrackedUsers.length === 0 ? (
              ...
              <ul className="flex flex-col gap-3">
                {currentTrackedUsers.map((usr) => (
                  <li
                    key={usr.id}
                    className="flex flex-col bg-neutral-900/60 p-4 rounded-xl border border-neutral-700"
                  >
                    ...
                    {usr.items.length > 0 ? (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {usr.items.map((item, i) => (
                          <span
                            key={i}
                            className="bg-neutral-800 text-neutral-300 text-xs px-2 py-0.5 rounded border border-neutral-700 capitalize"
                          >
                            {item}
                          </span>
                        ))}
